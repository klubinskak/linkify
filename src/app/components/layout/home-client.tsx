        ...
        >
          ...
          <div className="mb-16">
            <div className="flex items-center gap-2 mb-8">
              ...
              <h3 className="text-xl font-bold font-excon text-gray-200">Trending Now</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {isLoading ? (
                ...
                [...Array(3)].map((_, index) => (
                  <div
                    key={index}
                    className="bg-gray-800/30 backdrop-blur-sm p-6 rounded-2xl border border-gray-700/50 animate-pulse h-[160px]"
                  >
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="h-3 w-16 bg-gray-700/50 rounded mt-2"></div>
                        </div>
                      </div>
                    </div>
                ...
                trendingTools?.map((tool, index) => (
                 <Link href={`/${tool.topic}/${tool.subtopic}`} key={index}>
                   <motion.div
                    ...
                    className="bg-gray-800/30 backdrop-blur-sm p-6 rounded-2xl border border-gray-700/50 hover:border-gray-600/50 transition-all group cursor-pointer h-[160px]"
                  >
                    <div className="flex items-start mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="font-medium text-gray-200 group-hover:text-white transition-colors">{tool.title}</h4>
                          <p className="text-sm text-gray-400">{tool.subtopic}</p>
                        </div>
                      </div>
                    </div>
